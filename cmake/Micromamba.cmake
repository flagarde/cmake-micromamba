...
function(mamba_get_filename FILENAME)
  ...
  elseif(${CMAKE_HOST_SYSTEM_NAME} MATCHES "Linux")
    ...
    message(FATAL_ERROR "[cmake-micromamba] Micromamba is not available for your system \"${CMAKE_HOST_SYSTEM_NAME}\"")
  endif()
    ...
    if("${SHA256_EMPTY}" STREQUAL "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855")
      ...
      if(EXISTS "${MICROMAMBA_DESTINATION}/bin/micromamba")
        ...
        if(NOT "${MICROMAMBA_SHA256}" STREQUAL "${SHA256_TO_CHECK}")
          ...
          message(FATAL_ERROR "[cmake-micromamba] SHA256 of ${MICROMAMBA_DESTINATION}/bin/micromamba isn't equal to ${MICROMAMBA_SHA256}")
        endif()
      elseif(EXISTS "${MICROMAMBA_DESTINATION}/Library/bin/micromamba.exe")
        ...
        if(NOT "${MICROMAMBA_SHA256}" STREQUAL "${SHA256_TO_CHECK}")
          file(REMOVE_RECURSE "${MICROMAMBA_DESTINATION}")
          message(FATAL_ERROR "[cmake-micromamba] SHA256 of ${MICROMAMBA_DESTINATION}/Library/bin/micromamba.exe isn't equal to ${MICROMAMBA_SHA256}")
        endif()
      endif()
    endif()
  endif()
...
function(micromamba)
  ...
  if(NOT DEFINED MICROMAMBA_VERSION)
    message(FATAL_ERROR "[cmake-micromamba] VERSION must be set for download_micromamba !")
  endif()
  ...
  if(MICROMAMBA_STANDALONE AND MICROMAMBA_USE_LOCAL)
    message(FATAL_ERROR "[cmake-micromamba] STANDALONE and USE_LOCAL are mutually exclusive !")
  endif()
  ...
  if(NOT "${REPLACED_VERSION}" STREQUAL "${Micromamba_VERSION}")
    unset(Micromamba_VERSION CACHE)
    unset(Micromamba_EXECUTABLE CACHE)
    unset(Micromamba_FOUND CACHE)
  endif()
  ...
  elseif(MICROMAMBA_USE_LOCAL)
    ...
    if(NOT Micromamba_FOUND)
      message(STATUS "[cmake-micromamba] Installing micromamba version ${MICROMAMBA_VERSION} to ${MICROMAMBA_DESTINATION}")
      download_micromamba("${ARGN}")
      set(Micromamba_ROOT "${MICROMAMBA_DESTINATION}")
      find_package(Micromamba ${REPLACED_VERSION} REQUIRED MODULE)
    endif()
...
macro(micromamba_environment)
  message("[cmake-micromamba] Installing dependencies from channels :")
  cmake_parse_arguments(PARSED_ARGS "" "" "CHANNELS;DEPENDENCIES" ${ARGN})

  if("${Micromamba_EXECUTABLE}" STREQUAL "Micromamba_EXECUTABLE-NOTFOUND")
    message(FATAL_ERROR "[cmake-micromamba] Could NOT find Micromamba v${MICROMAMBA_VERSION} ! Use micromamba function !")
  endif()
  ...
  list(APPEND CMAKE_PREFIX_PATH "${CMAKE_BINARY_DIR}/environment" "${CMAKE_BINARY_DIR}/environment/Library")
